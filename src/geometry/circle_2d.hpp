...
bool is_intersect(const Circle& c, const Point& p) {
    return eq(abs(p - c.p), c.r);
}
bool is_intersect(const Circle& c, const Line& l) {
...
vector<Point> intersection(const Circle& c, const Line& l) {
    ...
    vector<Point> res;
    if(!is_intersect(c, l)) return res;
...
vector<Point> tangent(const Circle& c, const Point& p) {
    return intersection(c, Circle(p, sqrt(norm(p - c.p) - c.r * c.r)));
}
...
Real area(const vector<Point>& ps, const Circle& c) {
    int n = ps.size();
    if(n < 3) return 0.0;
    auto cross_area = [&](auto& cross_area, const Circle& c, const Point& a, const Point& b) -> Real {
        ...
        if(dist(Segment(a, b), c.p) > c.r - EPS) return c.r * c.r * arg(vb * conj(va));
        auto u = intersection(c, Segment(a, b));
